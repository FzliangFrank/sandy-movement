test_that("Tesing flows can be correctly generated when log is complete and valid", {
  # Parameter settings for fake data that's been generateed
  number_of_objects = 4
  maximum_status_change = 9
  reduce_period = 'month'
  fake_log=make_fakelog(number_of_objects, max_freq = maximum_status_change, n_error=0)
  
  ## create an audit log object and calculate net-flow
  netflow=try({
    fake_log |>
      new_auditLog(
  ...
  if(netflow |> inherits('try-error')) {
    fail('Message failed because netflow unexpectedly has not been generated')
  }
